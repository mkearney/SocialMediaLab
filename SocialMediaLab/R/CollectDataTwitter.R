...
#' @export
CollectDataTwitter <- function(searchTerm,
                               numTweets = 100,
                               verbose = TRUE,
                               writeToFile = FALSE,
                               language = NULL,
...
# cat(paste("DEBUG - numTweets is set to:", numTweets)) # DEBUG
  ...
  if (writeToFile) {
    ...
    save_as_csv(
      tweetsData,
      paste0(currTime, "_", searchTerm, "_TwitterData")
    )
    message("Twitter data was written to current working directory, with filename:\n")
    message(paste0(currTime,"_",searchTerm,"_TwitterData.csv"))
  }
  tweetsData
}
